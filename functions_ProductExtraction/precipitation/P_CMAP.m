function P_CMAP(indir,cmap_dir,outdir,ROIsites,extract_what)
    %% EXTRACT CMAP Land Precip DATA - TOTAL P [mm]
    %% CMAP : DAILY P - Total Precip [mm]
    % NETCDF

    fields = fieldnames(ROIsites);
    
    dat_files = dir([indir cmap_dir]);%dir([indir '\ERA5\PPT']);
            ...
            if contains([dat_files(i).folder '\' dat_files(i).name],'.nc')
                ...
                switch extract_what
                    case 'area'
                        ...
                        % save subset struct
                        out_dir = [outdir ROIsites.(char(fields(i_flds))).subset_nm '\' extract_what '\PPT_subset_' ROIsites.(char(fields(i_flds))).subset_nm '\CMAP\'];
                        if ~isfolder(out_dir), mkdir(out_dir),end
                        save([out_dir 'CMAP_Precip_' ROIsites.(fields{i_flds}).subset_nm '_' yr],'-struct','VAR')
