function P_CHIRP(indir,chirp_dir,outdir,ROIsites,extract_what)
    %% EXTRACT CHIRP Land Precip DATA - TOTAL P [mm]
    %% CHIRP : DAILY P - Total Precip [mm]
    % NETCDF
    
    fields = fieldnames(ROIsites);

    dat_files = dir([indir chirp_dir]);
            ...
            if contains([dat_files(i).folder '\' dat_files(i).name],'.nc')
                ...
                switch extract_what
                    case 'area'
                        ...
                        % save subset struct
                        out_dir = [outdir ROIsites.(char(fields(i_flds))).subset_nm '\' extract_what '\PPT_subset_' ROIsites.(char(fields(i_flds))).subset_nm '\CHIRP\'];
                        if ~isfolder(out_dir), mkdir(out_dir),end
                        save([out_dir 'CHIRP_Precip_' ROIsites.(fields{i_flds}).subset_nm '_' yr],'-struct','VAR')
