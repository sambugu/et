function ET_ERA5Land(indir,era5land_et_dir,outdir,ROIsites,extract_what)
    %% EXTRACT ERA5 Land ET DATA - TOTAL Evap [m]
    %% ERA5-Land : DAILY ET - Total Evap [m]
    % NETCDF
    fields = fieldnames(ROIsites);
    ERA5_Land_files = dir([indir era5land_et_dir]);
    ...
    %                     e = squeeze(nanmean(reshape(e,size(e,1),size(e,2),24,[]),3));
                        ...
                        % save subset struct
                        out_dir = [outdir ROIsites.(char(fields(i_flds))).subset_nm '\' extract_what '\ET_subset_' ROIsites.(char(fields(i_flds))).subset_nm '\ERA5_Land\'];
    ...
    %                     e = nanmean(reshape(e,24,[]));
                        ...
                        % save point struct
                        out_dir = [outdir ROIsites.(char(fields(i_flds))).site_nm '\' extract_what '\ET_' ROIsites.(char(fields(i_flds))).site_nm '\ERA5_Land\'];
                        if ~isfolder(out_dir), mkdir(out_dir),end
